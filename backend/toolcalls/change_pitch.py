    ...
    if not matched:
        ...
        for inst in midi.instruments:
            if inst.name and inst.name.strip().lower() == "singing":
                matched.append(inst)
        if matched:
            print(f"[change_pitch] Layer {layer!r} not found, defaulting to 'singing'")
    ...
    if not matched:
        ...
        for inst in midi.instruments:
            if not inst.is_drum:
                ...
                break
        if matched:
            print(f"[change_pitch] No named layers found, defaulting to first non-drum instrument ({matched[0].name!r})")
